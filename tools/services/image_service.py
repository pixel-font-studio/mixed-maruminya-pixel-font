...
def _load_font(font_size: FontSize, width_mode: WidthMode, language_flavor: LanguageFlavor, scale: int = 1) -> FreeTypeFont:
    file_path = path_define.outputs_dir.joinpath(f'mixed-maruminya-{font_size}px-{width_mode}-{language_flavor}.otf.woff2')
...
def make_preview_image(font_size: FontSize):
    ...
    image = Image.new('RGBA', ((font_size + 1) * 32, font_size * 2 + line_height * 9), (255, 255, 255, 255))
    _draw_text(image, (font_size, font_size), '混合咪喵像素圆体 / Mixed MaruMinya Pixel Font', font_zh_hans)
