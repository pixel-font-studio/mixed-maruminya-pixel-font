...
def make_release_zips(font_size: FontSize, width_mode: WidthMode, font_formats: list[FontFormat]):
    ...
    for font_format in font_formats:
        file_path = path_define.releases_dir.joinpath(f'mixed-maruminya-pixel-font-{font_size}px-{width_mode}-{font_format}-v{configs.version}.zip')
        with ZipFile(file_path, 'w') as file:
            ...
            for language_flavor in options.language_flavors:
                font_file_name = f'mixed-maruminya-{font_size}px-{width_mode}-{language_flavor}.{font_format}'
